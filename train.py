...
def dec_check(threshold, parameters, last, last_min):
    norm = 0
    dec = False
    for i, (name, param) in enumerate(parameters):
        temp = torch.sum((torch.abs(param.grad)))
        for j in range(len(param.shape)):
            temp = temp / param.shape[j]
            if temp > norm:
                norm = temp
    if torch.abs(norm/last-1) < threshold and torch.abs(norm/last_min-1) < threshold:
        dec = True
    if norm < last_min:
        last_min = norm
    
    return norm, last_min, dec
...
def run(graph, node_dict, gpb, args):
    ...
    if args.dataset == 'amazon' or 'yelp':
    ...
    if args.adap_dec: #intialize for adap_dec
        enabling_dec = True
        control = True
        dec = False
        count = 0
        last = 100000 #arbitrary large
        last_min = 100000 #arbitrary large
    ...
    for epoch in range(args.n_epochs):
        ...
        optimizer.step()
        if control == True:
            last, last_min, dec = dec_check(args.dec_threshold, model.named_parameters(), last, last_min)                
        if enabling_dec and dec:
            #start to decrease
            count += 1
            send_size = [int(size*(args.dec_ratio)) for size in send_size]
            send_data = [torch.tensor([d]) for d in send_size]
            recv_data_shape = [s.shape for s in send_data]
            recv_size_ori = data_transfer(send_data, recv_data_shape, tag=TransferTag.SIZE, dtype=torch.long)
            recv_size = []
            for i in range(len(recv_size_ori)):
                if recv_size_ori[i] == None:
                    recv_size.append(0)
                else:
                    recv_size.append(recv_size_ori[i].item())

            recv_shape = [torch.Size([s]) for s in recv_size]
            renorm = [decrease_ratio*norm for norm in ratio]
            if count > args.stop: #stop_decrease
                control = False
                enabling_dec = False
            ctx.buffer.init_buffer(in_graph.num_nodes(), renorm, send_size, recv_size, layer_size[:args.n_layers - args.n_linear], \
                                  use_pp=args.use_pp, backend=args.backend)
